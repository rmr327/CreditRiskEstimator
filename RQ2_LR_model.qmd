...
```{r}
credit_spread_mod <- lm(credit_spread ~ Sector + Duration + ticker + marketCapitalization + score + BB_COMPOSITE + debt_to_assets + debt_to_equity + int_coverage + cash_coverage + current_ratio + cash_ratio + roa + ebitda_margin, data=credit_risk_data)
...
```{r}
credit_spread_com <- lm(credit_spread ~ Duration + marketCapitalization + BB_COMPOSITE + roa + ebitda_margin + debt_to_assets + operating_profit_margin + score , data=credit_risk_data)
