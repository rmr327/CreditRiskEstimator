---
title: "Group Project: Exploration of Data"
...
3.  **Credit ratings from Bloomberg (4/34):** Credit ratings from Fitch, Moody's, and S&P, and a composite credit rating were sourced from [the Bloomberg Terminal](https://www.bloomberg.com/professional/solution/bloomberg-terminal/).
...
2.  Can we predict a certain bond's credit spread based on various metrics like the company's fundamentals and the market's sentiment related to that company? (Outcome Variable: cedit_spread)
...
plot1_capital <- ggplot(bonds, aes(x = marketCapitalization, y = credit_spread, col = Sector)) +
  geom_point() +
  geom_smooth(method = 'lm', se = F) +
...
The data set encompasses a range of additional fundamental technical indicators, including metrics like Rate of Amortization, EBITDA Margin, Operating Profit Margin, and others. Additionally, it incorporates sentiment score variables, such as the count of positive/negative mentions on Reddit over the past year, and the aggregate positive/negative sentiment scores within the same time frame. A correlation matrix is created to demonstrate the relationship between all numeric fundamentals variables.
...
Interest Coverage, Debt Service Coverage, and Cash Coverage provide valuable insights into a company's creditworthiness and financial stability. A stronger financial position, as indicated by higher ratios, tends to correlate with a lower credit risk, potentially leading to narrower credit spreads and more favorable credit ratings.

Cash ratio indicates a company's ability to pay off short-term liabilities with its cash and equivalents, while the current ratio measures its overall liquidity. These ratios are related to credit spread and credit rating because higher liquidity and solvency, reflected in these ratios, typically indicate lower credit risk, which may lead to narrower credit spreads and higher credit rating scores. Since they are highly correlated, using either cash ratio or current ratio can provide similar insights into a company's credit risk.
