...
plot <- ggplot(bonds, aes(x = debt_to_assets, y = credit_spread)) +
  geom_point() +
  theme_minimal() +
...
```{r, echo=FALSE}
plot2 <- ggplot(bonds, aes(x = debt_to_equity, y = credit_spread)) +
  geom_point() +
  theme_minimal() +
...
OThe dataset encompasses a range of additional fundamental technical indicators, including metrics like Rate of Amortization, EBITDA Margin, Operating Profit Margin, and others. Additionally, it incorporates sentiment score variables, such as the count of positive/negative mentions on Reddit over the past year, and the aggregate positive/negative sentiment scores within the same time frame. A correlation matrix is created to demonstrate the relationship between all numeric fundamentals variables.
