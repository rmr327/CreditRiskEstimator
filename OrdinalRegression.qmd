...
```{r}
# full model
ord_mod <- polr(BB_COMPOSITE ~ score + Sector + Duration + credit_spread + debt_to_assets + operating_profit_margin + ebitda_margin + roa + debt_to_equity, data=bonds, Hess=TRUE)
summary(ord_mod)
```
...
```{r}
df_r <- bonds[, c("BB_COMPOSITE", "score", "Sector",  "Duration", "credit_spread", "debt_to_assets", "operating_profit_margin", "ebitda_margin", "roa")]
...
df_r <- bonds[, c("BB_COMPOSITE", "score", "Duration", "credit_spread", "debt_to_assets", "operating_profit_margin", "ebitda_margin", "roa")]

# Define the reduced model (without Debt to Equity Ratio and Sector )
reduced_model <- polr(BB_COMPOSITE ~ score + 
                        Duration  + 
                        credit_spread + 
                        debt_to_assets + 
...
# Print as a pretty table with styling
kable(exp_coefs, format = "markdown", digits = c(3, 3, 3, 3), col.names = c("OR", "2.5%", "97.5%"), align = "c") %>%
  kable_styling(full_width = FALSE, position = "center", bootstrap_options = "striped")
```

```{r}
vif_model <- vif(reduced_model)
print(vif_model)
```

```{r}
# Plotting boxplot
boxplot(credit_spread ~ BB_COMPOSITE, data = bonds, xlab = "BB Composite", ylab = "Credit Spread",
        main = "Credit Spread vs BB Composite")

boxplot(roa ~ BB_COMPOSITE, data = bonds, xlab = "BB Composite", ylab = "Return on Assets (ROA)",
        main = "ROA vs BB Composite")

boxplot(Duration ~ BB_COMPOSITE, data = bonds, xlab = "BB Composite", ylab = "Duration",
        main = "Duration vs BB Composite")

boxplot(debt_to_assets ~ BB_COMPOSITE, data = bonds, xlab = "BB Composite", ylab = "Debt to Asset",
        main = "Debt to Asset vs BB Composite")
...
```{r}
df_filtered <- bonds[,c('BB_COMPOSITE', 'score', 'Duration', 'credit_spread', 'debt_to_assets', 'operating_profit_margin', 'ebitda_margin', 'roa')]
df_filtered <- na.omit(df_filtered)
confusionMatrix(predict(reduced_model), na.omit(df_filtered)$BB_COMPOSITE, mode = 'everything')
...
mult_mod <- multinom(BB_COMPOSITE ~ score + Duration + credit_spread + debt_to_assets + operating_profit_margin + ebitda_margin + roa, data = df_r)

df_r <- na.omit(df_r)

shuffled_data <- df_r[sample(nrow(df_r)), ]

# predict(mult_mod,  head(shuffled_data, 10), type="probs")
# predict(reduced_model, head(shuffled_data, 10), type="probs")

mult_prob <- as.data.frame(predict(mult_mod, head(shuffled_data, 10), type = "probs"))
mult_prob <- round(mult_prob, 5)  # Round to 5 decimal places

# Predicted probabilities from ordinal logistic regression
ord_prob <- as.data.frame(predict(reduced_model, head(shuffled_data, 10), type = "probs"))
ord_prob <- round(ord_prob, 5)  # Round to 5 decimal places
