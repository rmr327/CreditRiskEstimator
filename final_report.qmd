...
```{r}
# credit_spread vs duration
ggplot(subset(bonds,BB_COMPOSITE %in% c('A-', 'BBB+', 'BBB')), aes(x = Duration, y = credit_spread)) + geom_point() + facet_wrap(~BB_COMPOSITE, scales = 'free') + geom_smooth(method = 'lm', se = F) + labs(x = 'Duration', y = 'Credit spread (%)', title = 'Credit spread vs Duration', subtitle = 'How does interest rate risk affect credit spread?')
```

```{r}
# rating vs sentiment
ggplot(bonds, aes(x = BB_COMPOSITE, y = score)) + geom_boxplot() + labs(x = 'Credit rating', y = 'Sentiment score', title = 'Sentiment score vs Credit rating', subtitle = 'What is the market\'s appetite for risk?') 
```
