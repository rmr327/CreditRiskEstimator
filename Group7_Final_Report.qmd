...
A Priori variable selection was done to make the initial model. The A priori variables were selected based on our EDA and domain knowledge.

```{r}
# full model
ord_mod <- polr(BB_COMPOSITE ~ score + Duration + credit_spread + debt_to_assets + operating_profit_margin + ebitda_margin + roa + debt_to_equity, data=bonds, Hess=TRUE)
summary(ord_mod)
```
...
reduced_model <- polr(BB_COMPOSITE ~ score + Duration + credit_spread + debt_to_assets + operating_profit_margin + ebitda_margin + roa, data = df_r, Hess=TRUE)
